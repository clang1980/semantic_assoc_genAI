...
prolific_agree_item = ggplot(prolific_distribution_wide, aes(y=agreement, x=maj_vote)) +
  ...
  geom_text_repel(aes(label=label_scatter)) +
  labs(
...
openAI_agree_item = ggplot(gpt4o_res_comb_df, aes(y=agreement, x=maj_vote)) + 
  geom_jitter(width = 0.1, height = 0.0) +
  geom_text_repel(aes(label=label_scatter)) +
  labs(subtitle = paste0("GPT4o; labels for agreement < ", openAI_cutoff),
       x = "Majority Vote",
       y = "Agreement per Item") +
  theme(
    axis.title.y = element_text(color = "black", size = 12),
...
# data = prolific_distribution_wide_llm_sub_1
...
hist_openAI <- ggplot(openAI_assoc_no_agg, aes(x=Association, fill=Label)) +
  ...
  theme(legend.position="top") +
  labs(
    # subtitle = paste0("GPT4o; N (total):", length(openAI_assoc_no_agg$Association), "; N (Label m): ", length(openAI_assoc_no_agg[which(openAI_assoc_no_agg$Label=="m"),]$Association), "; N (Label w): ",  length(openAI_assoc_no_agg[which(openAI_assoc_no_agg$Label=="w"),]$Association)),
    subtitle = paste0("GPT4o; N (total): ", length(openAI_assoc_no_agg$Association)),
...
openAI_assoc_logprob_fig <- ggplot(openAI_assoc_no_agg, aes(x = Logprobs_Sum, y = Association, shape = Label, color = Label)) +
  ...
  # labs(subtitle = paste0("GPT4o; N (total):", length(openAI_assoc_no_agg$Association), "; N (Label m): ", length(openAI_assoc_no_agg[which(openAI_assoc_no_agg$Label=="m"),]$Association), "; N (Label w): ",  length(openAI_assoc_no_agg[which(openAI_assoc_no_agg$Label=="w"),]$Association)),
  labs(subtitle = paste0("GPT4o; N (total): ", length(openAI_assoc_no_agg$Association)),
...
OpenAI_agreement_logprobs_fig <- ggplot(gpt4o_res_comb_df, aes(x = logprobs_mean, y = agreement)) +
  ...
  labs(#title = "Label Agreement and Mean Surprisal",
    subtitle = paste0("GPT4o; N = ", length(gpt4o_res_comb_df$np)),
